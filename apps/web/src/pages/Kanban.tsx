...
export default function Kanban() {
  ...
  return (<div>
    ...
    {show && <div className="bg-white rounded-2xl border border-border p-4 mb-6 flex gap-3">
      <input value={title} onChange={e => setTitle(e.target.value)} onKeyDown={e => e.key==='Enter' && (async () => { await api('/tasks',{method:'POST',body:JSON.stringify({title,teamId:'',status:'todo'})}); setTitle(''); setShow(false); })()} placeholder="Tên task" autoFocus className="flex-1 px-4 py-2.5 bg-surface border border-border rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#4f46e5]/25" />
